...
export function buildWechatLayout(input: { selectedTitle: string; summary: string; sections: Array<{ sectionKey: string; heading: string; markdown: string }>; brand?: BrandConfig }): ContentLayoutNode[] {
  const { brand } = input;
  const resolvedBrand = brand ?? DEFAULT_BRAND;
  const nodes: ContentLayoutNode[] = [
    { id: "brand-bar", type: "brand_bar", text: resolvedBrand.name },
